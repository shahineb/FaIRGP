...
def compute_I(scenario_dataset, kernel, q_map, d_map):
    I = [compute_I_scenario(scenario_dataset, scenario, kernel, q_map, d_map)
         for scenario in scenario_dataset.scenarios.values()]
    I = torch.cat(I, dim=-3)
    return I


def compute_I_scenario(scenario_dataset, scenario, kernel, q_map, d_map):
...
def compute_covariance(scenario_dataset, I, q_map, d_map):
    Kj = [compute_covariance_scenario(scenario_dataset, scenario, I, q_map, d_map)
          for scenario in scenario_dataset.scenarios.values()]
    Kj = torch.cat(Kj, dim=-3)
    Kj = scenario_dataset.trim_hist(Kj).sum(dim=-1)
    Kj = 0.5 * (Kj + Kj.permute(1, 0, 2))
    return Kj


def compute_covariance_scenario(scenario_dataset, scenario, I, q_map, d_map):
    d = d_map.view(2, -1)
    q = q_map.view(2, -1)
