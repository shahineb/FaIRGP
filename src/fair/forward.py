"""
Code adpted from https://github.com/OMS-NetZero/FAIR
"""
import os
import sys
import numpy as np
    ...
    **kwargs
    ...
    for i, tstep in enumerate(timestep):
        ...
        RF[..., i] = tools.step_forcing(
            C=C[..., i], PI_conc=PI_conc, f1=f1, f2=f2, f3=f3, f1aci=f1aci, f2aci=f2aci, PI_SO2=PI_SO2, emissions=inp_ar[..., i], use_aci=use_aci,
        )
...
def _run_concentration_driven(
    ...
    **kwargs
    ...
    n_species, n_timesteps = inp_ar.shape
    # Concentration, Radiative Forcing and Alpha
    RF = np.zeros((n_species, n_timesteps))
    # Temperature
    T = np.zeros(n_timesteps)
    # S represents the results of the calculations from the thermal boxes,
    # an Impulse Response calculation (T = sum(S))
    ...
    for i, tstep in enumerate(timestep):
        RF[..., i] = tools.step_forcing(
            C=inp_ar[..., i], PI_conc=PI_conc, f1=f1, f2=f2, f3=f3, f1aci=f1aci, f2aci=f2aci, PI_SO2=PI_SO2, emissions=inp_ar[..., i], use_aci=use_aci,
        )
        S[i], T[i] = tools.step_temperature(
            S_old=S[max(i - 1, 0)], F=np.sum(RF[..., i], axis=0) + ext_forcing[i], q=q, d=d, dt=tstep
        )
